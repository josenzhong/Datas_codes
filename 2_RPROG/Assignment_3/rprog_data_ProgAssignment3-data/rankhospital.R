rankhospital <- function(state, outcome, num = "best") {
        ...
        if(!(state %in% outcome.short$State)){
                ...
                outcome.order.decrease <- outcome.ss[order(-outcome.ss$heart.attack,
                                                           outcome.ss$Hospital), ]
                length <- nrow(outcome.order)
                ...
                outcome.order.decrease <- outcome.ss[order(-outcome.ss$heart.failure,
                                                           outcome.ss$Hospital), ]
                length <- nrow(outcome.order)
                ...
                outcome.order.decrease <- outcome.ss[order(-outcome.ss$pneumonia,
                                                           outcome.ss$Hospital), ]
                length <- nrow(outcome.order)
        ...
        if(num == "best") {
                ...
                x <-as.matrix(outcome.order.decrease[1, ])
                x[, "Hospital"]
